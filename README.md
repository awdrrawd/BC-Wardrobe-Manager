...
## Credits & Licenses / 致謝與授權

This project **does not include** the Bondage Club or ECHO source code or assets.
It only **references / interoperates** with them at runtime. All rights belong to
their respective authors:

- **Bondage Club** — https://github.com/Ben987/Bondage-College
- **ECHO Clothing (`echo-clothing-ext`)** — https://github.com/SugarChain-Studio/echo-clothing-ext

本專案**不包含** Bondage Club 或 ECHO 的原始碼與素材，僅在執行時**引用／互通**。
版權歸各自作者所有（連結同上）。
...
The BC source is **not bundled in this repo** — you must place it next to the repo
yourself. 本專案**不含** BC 原始碼，請自行放到 repo 旁邊。

```
parent-folder/
├── BC-Wardrobe-Manager/     ← this repo / 本倉庫
└── Bondage-College-master/  ← BC source you download yourself / 你自行下載的 BC 原始碼
```

1. Download BC source from https://github.com/Ben987/Bondage-College (the `master`
   zip extracts to a `Bondage-College-master/` folder) and put it **one level above**
   this repo. 下載 BC 原始碼（master zip 解壓後資料夾名為 `Bondage-College-master`），放到本 repo 的**上一層**。
2. `npm install`
3. `npm start`

> `main.js → getBCPath()` 開發時讀取 `../Bondage-College-master/BondageClub`；打包（`package.json`
> 的 `extraResources`）也從那裡複製所需的 BC 腳本。ECHO 於執行時從網路載入，開發**不需**本機 ECHO；
> 若要研究其行為，可另行 clone `echo-clothing-ext`。
>
> Build: see `TECHNICAL.md` (portable build is recommended; the NSIS installer needs
> Windows Developer Mode due to electron-builder's `winCodeSign`).
